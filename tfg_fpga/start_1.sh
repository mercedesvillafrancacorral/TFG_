#!/bin/bash
# Levanta toda la aplicación (Linux server o Windows local, vía Git Bash)
# Uso: bash start_1.sh
...
# ─── 0. Detección de sistema operativo ──────────────────────────────────────
case "$(uname -s)" in
...
if [ "$OS" = "windows" ]; then
    ...
    GRAFANA_PORT=3001   # mapeado por docker-compose.yml (3001:3000)
    SUDO=""
else
    VENV_BIN="$VENV/bin"
    GRAFANA_PORT=3000
...
cleanup() {
    ...
    if [ -n "$API_PID" ]; then
        ...
        [ "$OS" = "linux" ] && sudo pkill -f "uvicorn main:app" 2>/dev/null
    fi
}
trap cleanup EXIT
...
# ─── 2. FPGA (solo aplica en el servidor Linux con la FPGA conectada) ───────
if [ "$OS" = "windows" ]; then
    if [ -n "$UART_PORT" ]; then
        ok "Puerto serie indicado manualmente: $UART_PORT (no se verifica su existencia en Windows)"
    else
        info "En Windows no hay FPGA conectada localmente. Arrancando en modo SIMULACIÓN."
        SIM_MODE=true
    fi
else
...
if curl -s "http://localhost:$ES_PORT" > /dev/null 2>&1; then
    ...
    command -v docker >/dev/null 2>&1 || err "Docker no está instalado. Instala Docker Desktop:\n  https://www.docker.com/products/docker-desktop"
    info "Arrancando Elasticsearch vía Docker Compose..."
    docker compose -f "$SCRIPT_DIR/docker-compose.yml" up -d elasticsearch
...
if curl -s "http://localhost:$GRAFANA_PORT" > /dev/null 2>&1; then
    ...
    info "Arrancando Grafana vía Docker Compose..."
...
# ─── 4. Kibana (opcional, no bloquea el arranque) ───────────────────────────
if [ "$OS" = "windows" ]; then
    info "Kibana no está provisionado en docker-compose.yml para Windows; se omite (añádelo si lo necesitas)."
else
    KIBANA_PORT=5601
